...
const bootRepos = ["getCultured", 'techBlog', "JAKL-drag-n-drop"]
...
function App() {
  ...
  useEffect(() => {
      window.addEventListener("resize", () => {setBurgerOpen(false); setScreenwidth(document.body.clientWidth)});
  }, []);
  ...
  useEffect(() => {
    const getRepoData = async(repoArray) => {
      const data = await Promise.all(repoArray.map(async (repoName) => {
  ...
  return (
    <div className='w-screen h-screen overflow-y-hidden'>
      ...
      <Project pageInView={pageInView} repos={[ metaRepoData, bootRepoData]} screenwidthProp={screenwidth} />
      <Footer />
    </div>
  );
}
