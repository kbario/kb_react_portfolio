...
function RepoSection({repo}) {
    ...
    return (
        <article className="flex flex-col rounded-sm bg-zinc-100 border" >
            {repo.img !== undefined && <div style={{backgroundImage: "url(" + repo.img +")"}} className="w-full h-56 bg-cover bg-center bg-no-repeat rounded-t-sm bg-gradient-to-t from-zinc-900 to-transparent"></div>}
            {repo.img !== undefined && <hr />}
            <div className="w-full flex p-3 h-64">
                <div className="flex flex-col gap-3 w-full">
                    <div className="flex justify-between">
                        <h3 className="text-xl">{repo.name}</h3>
                        <img height="20" width="20" className="fill-zinc-300" src={`https://unpkg.com/simple-icons@v6/icons/${lang.toLowerCase()}.svg`} alt="" />
                    </div>
                    <ul className="flex gap-1 flex-wrap">
                        {repo.topics.map((topic) => (
                            <li className="rounded-sm bg-zinc-200 hover:bg-zinc-300 hover:cursor-default px-2 text-sm font-mukta" key={topic}>{topic}</li>
                        ))}
                    </ul>
                    <p className="text-md text-zinc-600 grow">{repo.description}</p>
                    <div className="flex justify-center gap-3">
                        <a className="blue-btn flex gap-2 items-center justify-center" href={repo.homepage} target="_blank" rel="noopener noreferrer">
                            <h4>Deployed App</h4>
